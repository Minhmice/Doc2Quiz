...
-- Some deployed projects predate the v2.1 stage contract. CREATE TABLE IF NOT
-- EXISTS does not update their CHECK constraint or column default.
lock table public.study_sets in share row exclusive mode;
...
where study_set.pipeline_stage is null
   or study_set.pipeline_stage not in (
     'input',
     'raw',
     'canonical',
     'mode_selected',
