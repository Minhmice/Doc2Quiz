...
revoke all on function public.create_study_challenge(uuid,uuid,text,timestamptz,text,text),public.start_study_challenge_attempt(uuid),public.accept_study_challenge(uuid),public.get_study_attempt_practice(uuid),public.complete_study_attempt(uuid,jsonb,integer),public.sweep_study_challenge_reminders(),public.remove_friend(uuid) from public,anon;
grant execute on function public.create_study_challenge(uuid,uuid,text,timestamptz,text,text),public.start_study_challenge_attempt(uuid),public.accept_study_challenge(uuid),public.get_study_attempt_practice(uuid),public.complete_study_attempt(uuid,jsonb,integer),public.sweep_study_challenge_reminders(),public.remove_friend(uuid) to authenticated;
