...
begin
  ...
  if jsonb_typeof(p_events) <> 'array' or jsonb_array_length(p_events) > 200 then raise exception 'invalid_activity_batch'; end if;
  if exists (
    ...
    where item ?| array['body', 'token', 'ip', 'study_action']
       ...
       or item->>'eventId' is null or item->>'userId' is null or item->>'occurredAt' is null or item->>'dedupeKey' is null
  ...
  with input as (
    ...
    from inserted order by user_id, activity_kind, date_trunc('minute', occurred_at), occurred_at desc
  ), upserted as (
    insert into private.social_activity (user_id, last_active_at)
    select user_id, max(occurred_at) from newest group by user_id
    on conflict (user_id) do update set last_active_at = greatest(private.social_activity.last_active_at, excluded.last_active_at)
    returning 1
  ) select count(*) into v_inserted from upserted;
  delete from private.social_activity_events where created_at < now() - interval '24 hours';
  return v_inserted;
end;
$$;
