...
**Status:** Complete — `33-01-SUMMARY.md`, `33-02-SUMMARY.md`, `33-03-SUMMARY.md` (closed 2026-04-18)
...
Plans:
- [x] `33-01-PLAN.md` — IDs + ROADMAP reconcile; `embeddingIndexDb` (or equivalent) schema; `POST /api/ai/embed` (or forward extension); types; caps constants; cosine similarity helper (RAG-33-01..04)
- [x] `33-02-PLAN.md` — Indexing jobs: chunk text → embed → store; study-set scope; invalidation on model/dim change; pipelineLog (RAG-33-03, RAG-33-04)
- [x] `33-03-PLAN.md` — UI: semantic search panel + inject toggle / apply to `parseChunkOnce` / `runSequentialParse`; untrusted text UI; `npm run lint` + `npm run build` (RAG-33-05..08)
...
**Status:** Complete — `35-01-SUMMARY.md`, `35-02-SUMMARY.md`, `35-03-SUMMARY.md` (2026-04-18)
...
Plans:
- [x] `35-01-PLAN.md` — Constants module + `ocrRasterPreprocess.ts` skeleton; ROADMAP/REQUIREMENTS reconcile (wave 1)
- [x] `35-02-PLAN.md` — Integrate downsample/threshold into `renderPagesToImages` + OCR path (wave 2)
- [x] `35-03-PLAN.md` — `pipelineLog` batch summary; `npm run build`; `35-03-SUMMARY.md` (wave 3)

### Phase 36: Queue-based fallback to high-accuracy pipeline only on uncertain docs

**Status:** Complete — `36-01-SUMMARY.md`, `36-02-SUMMARY.md`, `36-03-SUMMARY.md` (2026-04-18; **FB-36-03** second-pass vision deferred)
...
**Requirements**: FB-36-01..FB-36-07 (**FB-36-03** open until relaxed second pass ships)
**Depends on:** Phase 35
**Plans:** 3/3 plans complete (minimal v1: registry + signals + placeholder runner)

Plans:
- [x] `36-01-PLAN.md` — Uncertainty types + `uncertainParseFallback.ts` queue skeleton (wave 1)
- [x] `36-02-PLAN.md` — `AiParseSection` enqueue + second-pass runner (wave 2)
- [x] `36-03-PLAN.md` — Minimal UX toast/summary; build; `36-03-SUMMARY.md` (wave 3)

### Phase 37: Global transfer acceleration / edge ingress

**Status:** Complete — `37-01-SUMMARY.md`, `37-02-SUMMARY.md`, `37-03-SUMMARY.md` (2026-04-18)
...
Plans:
- [x] `37-01-PLAN.md` — `docs/DEPLOY-edge-and-upload.md` + traceability (wave 1)
- [x] `37-02-PLAN.md` — `next.config.ts` / middleware safe cache headers (wave 2)
- [x] `37-03-PLAN.md` — `pdfUploadClient` regional URL tuning; `37-03-SUMMARY.md` (wave 3)

### Phase 38: Full fine-tuning or distillation for quiz style

**Status:** Complete — `38-01-SUMMARY.md`, `38-02-SUMMARY.md`, `38-03-SUMMARY.md` (2026-04-18)
...
Plans:
- [x] `38-01-PLAN.md` — `38-RESEARCH.md` go/no-go (wave 1)
- [x] `38-02-PLAN.md` — Eval criteria doc + explicit JSONL export from Settings (wave 2)
- [x] `38-03-PLAN.md` — Optional `scripts/eval-export-smoke.mjs`; build; `38-03-SUMMARY.md` (wave 3)
