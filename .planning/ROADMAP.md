...
Plans:
- [x] `31-01-PLAN.md` — ROADMAP/REQUIREMENTS IDs; IDB store design, key derivation API, prompt bundle alignment + LRU caps (completed 2026-04-18)
- [x] `31-02-PLAN.md` — Vision batch cache: memory Map → IndexedDB with fallback; wire `runVisionBatchSequential` (completed 2026-04-18)
- [x] `31-03-PLAN.md` — Text + layout single-MCQ cache in `parseChunk` / `runSequentialParse`; minimal pipelineLog summary; lint + build (completed 2026-04-18)
...
Plans:
- [x] `32-01-PLAN.md` — ROADMAP/REQUIREMENTS reconcile; validator prompt bundle + reason codes; `ParseCacheLane` / key strategy for validator pass; `mcqDraftValidate` module skeleton (completed 2026-04-18)
- [x] `32-02-PLAN.md` — Wire `parseChunkOnce` / `parseChunkSingleMcqOnce` / `runSequentialParse`: draft → validate → merge; pipelineLog reason codes; abort-safe (completed 2026-04-18)
- [x] `32-03-PLAN.md` — `AiParseSection` toast (throttled); vision/flashcard batch path where applicable; `npm run lint` + `npm run build` (completed 2026-04-18)
