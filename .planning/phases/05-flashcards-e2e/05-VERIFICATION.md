...
overrides_applied: 0
human_verification:
  - test: "Flashcards express path — canonical preview → Flashcards CTA → 3-step wizard → generate → /flashcards/[id] practice → /flashcards/[id]/done"
    expected: "Wizard collects goal/coverage/amount; progress shows recommendedCount, generatedCount, detectedFormat; session loads cards; Space flips; Done routes to done page; dashboard shows Start flashcards"
...
| 3 | User can start flashcard learning session | ✓ VERIFIED | `CanonicalModeSelectionFooter` enables Flashcards CTA + resume strip. Source page state machine: wizard → `postFlashcardGenerate` → redirect `flashcardsPlay(id)`. `FlashcardSession` loads `getApprovedFlashcardBank`; Space flip + Done → `flashcardsDone`. Dashboard `studySetActionLabels` returns "Start flashcards" for `content_kind=flashcards`. |
| 4 | Mistakes drill works for quiz sessions | ✓ VERIFIED | `recordQuizCompletion` upserts/deletes `study_wrong_history`. `QuizSession` filters via `getMistakeQuestionIds` when `reviewMistakesOnly`. `quiz/[id]/page.tsx` reads `?review=mistakes`. Dashboard + quiz done pages link to `?review=mistakes` when `hasMistakesForStudySet`. `activityTracking.test.ts` covers write path. |
...
| `src/app/(app)/sets/[id]/source/page.tsx` | Flashcard state machine | ✓ VERIFIED | Wizard, generate, redirect to `/flashcards/[id]` |
| `src/components/flashcards/FlashcardSession.tsx` | Practice session | ✓ VERIFIED | Loads bank; keyboard Space flip; Done routing |
| `src/components/dashboard/DashboardStudySetCard.tsx` | Dashboard CTAs | ✓ VERIFIED | "Start flashcards" via labels; "Drill mistakes" quiz-only |

### Key Link Verification

| From | To | Via | Status | Details |
|------|-----|-----|--------|---------|
| `source/page.tsx` | `/api/study-sets/[id]/flashcards/generate` | `postFlashcardGenerate` | ✓ WIRED | Line 228: `postFlashcardGenerate(id, body)` |
| `FlashcardSession.tsx` | `studySetDb.ts` | `getApprovedFlashcardBank` on mount | ✓ WIRED | `reload()` calls bank fetch in `useEffect` |
| `DashboardStudySetCard.tsx` | `/quiz/{id}?review=mistakes` | `reviewMistakesHref` when `hasMistakes` | ✓ WIRED | Quiz-only guard: `meta.contentKind === "quiz"` |
...
| `DashboardStudySetCard` | `hasMistakes`, `primaryLabel` | `useDashboardHome` → `hasMistakesForStudySet`, `getStudySetActionLabel` | Yes (parallel fetches per set) | ✓ FLOWING |
...
**Expected:** Progress shows counts/format; cards load at `/flashcards/{id}`; Space flips; Done → `/flashcards/{id}/done`; dashboard shows "Start flashcards"; source resume strip appears.  
