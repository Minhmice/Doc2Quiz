...
import { quizEdit, quizDrillMistakes, quizPlay, quizResults } from "@/lib/routes/studySetPaths";
...
export function PlaySession({
  ...
  const copy = messages.workflows.practice.quiz;
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      setPicked(null);
      setCorrectCount(0);
      const unfinished = (await listUnfinishedStudySessions()).find((session) => session.studySetId === studySetId && session.mode === "quiz" && session.practice === (reviewMistakesOnly ? "mistakes" : "standard"));
      const session = unfinished ?? await startStudySession({ studySetId, mode: "quiz", practice: reviewMistakesOnly ? "mistakes" : "standard", itemIds: list.map((q) => q.id), currentItemId: list[0]?.id ?? null, nextItemId: list[0]?.id ?? null, interaction: { mode: "quiz", answers: {} } });
      setDurableSession(session);
      if (session) {
        const restoredIndex = session.currentItemId ? Math.max(0, list.findIndex((q) => q.id === session.currentItemId)) : 0;
        setIndex(restoredIndex);
        const answer = session.interaction.mode === "quiz" ? session.interaction.answers[list[restoredIndex]?.id ?? ""] : undefined;
        setPicked(answer?.selectedIndex as 0 | 1 | 2 | 3 | null ?? null);
        setCorrectCount(Object.values(session.interaction.mode === "quiz" ? session.interaction.answers : {}).filter((entry) => entry.correct).length);
      }
    } catch (e) {
      setLoadError(
  ...
  useEffect(() => {
    ...
    void recordQuizCompletion({ studySetId, totalQuestions: playable.length, correctCount, wrongQuestionIds: [...wrongIdsRef.current] });
    if (durableSession) void completeStudySession({ ...durableSession, nextItemId: null });
  }, [finished, playable.length, correctCount, studySetId, durableSession]);
  ...
  useEffect(() => {
    ...
    router.push(quizResults(studySetId));
  }, [router, stitch, finished, studySetId]);

  const goNext = useCallback(() => {
    if (picked === null || current === undefined) return;
    const isCorrect = picked === current.correctIndex;
    if (!isCorrect) wrongIdsRef.current.add(current.id); else setCorrectCount((c) => c + 1);
    const nextIndex = index + 1;
    const answers = durableSession?.interaction.mode === "quiz" ? { ...durableSession.interaction.answers, [current.id]: { selectedIndex: picked, correct: isCorrect } } : { [current.id]: { selectedIndex: picked, correct: isCorrect } };
    const nextItemId = playable[nextIndex]?.id ?? null;
    if (durableSession) void saveStudySession(durableSession, { currentItemId: playable[nextIndex]?.id ?? null, nextItemId, interaction: { mode: "quiz", answers } }).then(({ session }) => { if (session) setDurableSession(session); });
    setPicked(null); setIndex(nextIndex);
  }, [picked, current, index, playable, durableSession]);
    ...
    const resultsCard = (
      <Card className={cn(resultsCardClass)}>
        ...
        >
          {wrongCount > 0 ? (
            <Link
              href={quizDrillMistakes(studySetId)}
    ...
    return (
      ...
      >
        ...
        <div className="space-y-4">
          ...
          {revealed ? (
            <div role="status" className="rounded-sm border border-[color-mix(in_srgb,var(--qp-outline-variant)_28%,transparent)] bg-[var(--qp-surface-container-low)] p-4">
              <Badge variant="secondary">{answerCorrect ? messages.correct.label : messages.wrong.label}</Badge>
              {current.explanation ? <MathText source={current.explanation} className="mt-3 text-sm leading-relaxed" /> : null}
              <LocalizedSlangLine context={answerCorrect ? "correct" : "wrong"} eventKey={`${current.id}:${index}:reveal`} className="mt-2 text-sm text-[var(--qp-muted)]" />
            </div>
          ) : null}
          ...
          <div className="flex flex-col gap-3 pt-1 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex-1">
              <QuizInteractionHints
                items={[
                  { key: "1–4", label: copy.choose },
                  { key: "← →", label: copy.navigation },
                ]}
            ...
            {revealed ? (
              ...
              >
                {index + 1 >= playable.length ? copy.seeResults : copy.next}
              </Button>
            ) : null}
          </div>
        </div>
