import { getChatCompletionsUrl, getAiRequestTimeoutMs } from "@/lib/server/ai-processing-config";
...
type PostArgs = {
  ...
  seed?: number;
  /** Caps completion length; defaults to 16384 when omitted. */
  max_tokens?: number;
  signal?: AbortSignal;
};
...
export async function postChatCompletionAssistantText(
  ...
  const signal = args.signal ?? AbortSignal.timeout(getAiRequestTimeoutMs());
