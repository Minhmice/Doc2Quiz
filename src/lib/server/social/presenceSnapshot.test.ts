...
import { getPresenceSnapshot } from "./presenceSnapshot";
...
describe("presence snapshots", () => {
  it("maps healthy sessions and durable ages to canonical buckets", async () => {
    ...
    const snapshot = await getPresenceSnapshot(redis, () => now, new Map()).snapshot(page, "online", "a");
  ...
  it("uses bounded last-known data then unknown without reclassifying offline", async () => {
    ...
    await getPresenceSnapshot(healthy, () => now, cache).snapshot(page, "online", "a");

    const withinGrace = await getPresenceSnapshot(null, () => now + 15_000, cache).snapshot(page, "online", "a");
    expect(withinGrace.items).toEqual([expect.objectContaining({ presence: "online", source: "last_known" })]);

    const expired = await getPresenceSnapshot(null, () => now + 15_001, cache).snapshot(page, "offline", "a");
