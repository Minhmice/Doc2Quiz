...
/** Default upstream chat timeout (ms). Override with AI_REQUEST_TIMEOUT_MS in .env */
...
/** Server-only chat/completions fetch timeout in milliseconds. */
export function getAiRequestTimeoutMs(): number {
  const raw = trimEnv("AI_REQUEST_TIMEOUT_MS");
  if (!raw) {
